...
void Tracker::AssociateDetectionsToTrackers(const std::vector<cv::Rect> &detection,
    ...
    if (tracks.empty()) {
        ...
        for (const auto &det : detection) {
            unmatched_det.emplace_back(i++, det);
        }
        return;
    }
    ...
    for (size_t i = 0; i < detection.size(); i++) {
        ...
        if (!matched_flag) {
            unmatched_det.emplace_back(i, detection[i]);
        }
    }
}


std::vector<int> Tracker::Run(const std::vector<cv::Rect> &detections) {
    std::vector<int> match_ids(detections.size());
    ...
    if (!detections.empty()) {
        AssociateDetectionsToTrackers(detections, tracks_, matched, unmatched_det, match_ids);
    }
    ...
    for (const auto &det : unmatched_det) {
        ...
        int new_id = id_++;
        tracks_[new_id] = tracker;
        match_ids[det.first] = new_id;
    }
