...
__global__ void sigmoidKernel(__half * input, __half * output, int
edge) {
...
inline void segemm_native(cublasHandle_t handle,
    ...
    cublasCheck(
            cublasGemmEx(handle, transa, transb, m, n, k, &halpha, A, CUDA_R_16F, lda, B, CUDA_R_16F, ldb, &hbeta, C,
                         CUDA_R_16F, ldc, CUDA_R_16F, CUBLAS_GEMM_DFALT));
}

template<typename DataType>
static void enqueue_native(cublasHandle_t handle, const std::vector<GTensor> &inputs, std::vector<GTensor> &outputs,
                           const std::vector<GTensor> &weights, void *workspace, cudaStream_t stream) {
    auto &data = inputs[0];
    auto &om = inputs[1];
    auto &out = outputs[0];
    ...
    for (int ibatch = 0; ibatch < data.batch(); ++ibatch) {
        ...
        checkCudaKernel(
                sigmoidKernel<<<CUDATools::grid_dims(maskSize), CUDATools::block_dims(maskSize), 0, stream>>>(inputMask,
                                                                                                              maskWorkspacePtr,
                                                                                                              maskSize);
        );
        ...
        checkCudaKernel(
                DCNIm2colKernel<<<CUDATools::grid_dims(jobs), CUDATools::block_dims(jobs), 0, stream>>>(
                        datainput, offset, maskWorkspacePtr, data.height(), data.width(), kernel_size, kernel_size, 1,
                        1, 1, 1, 1, 1, data.channel(), data.batch(), data.channel(), deformable_group,
                        out.height(), out.width(), im2colWorkspacePtr, jobs
                );
        );

        DataType *weightKernel = weights[0].ptr<DataType>();
        segemm_native(handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, alpha, im2colWorkspacePtr, n, weightKernel, k, beta,
                      out.ptr<DataType>(ibatch), n);
...
public:
    ...
    virtual void attachToContext(cudnnContext * /*cudnn*/, cublasContext *cublas,
    ...
    size_t getWorkspaceSize(const nvinfer1::PluginTensorDesc *inputs, int32_t nbInputs,
        ...
        int deformable_group = 1;
        size_t im2colSize =
                (size_t) inputs[0].dims.d[1] * kernel_size * kernel_size * outputs[0].dims.d[2] * outputs[0].dims.d[3];
        size_t maskSize =
                (size_t) inputs[0].dims.d[2] * inputs[0].dims.d[3] * kernel_size * kernel_size * deformable_group;
        config_->workspace_size_ =
                (im2colSize + maskSize) * config_->max_batch_size_ * TRT::data_type_size(config_->usage_dtype_);
    ...
    int enqueue(const std::vector<GTensor> &inputs, std::vector<GTensor> &outputs, const std::vector<GTensor> &weights,
