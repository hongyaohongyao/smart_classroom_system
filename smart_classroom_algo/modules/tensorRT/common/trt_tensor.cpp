...
namespace TRT{
	...
	Tensor& Tensor::copy_from_gpu(size_t offset, const void* src, size_t num_element, int device_id){
		...
		if(head_ == DataHead::Device){
			...
			if(current_device_id != gpu_device_id){
				...
				//checkCudaRuntime(cudaMemcpyAsync(gpu<unsigned char>() + offset_location, modules, copyed_bytes, cudaMemcpyDeviceToDevice, stream_));
			}
			else{
	...
	Tensor& Tensor::copy_from_cpu(size_t offset, const void* src, size_t num_element){
		...
		if(head_ == DataHead::Device){
			...
			//checkCudaRuntime(cudaMemcpyAsync((char*)data_->cpu() + offset_location, modules, copyed_bytes, cudaMemcpyHostToHost, stream_));
